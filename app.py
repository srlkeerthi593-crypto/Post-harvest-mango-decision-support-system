...
if st.session_state.run_analysis:
    ...
    for category, df in category_dfs.items():
        ...
        name_col = "market" if category=="Mandi" else detect_name(df)
    ...
    # ---------------- MAP SHOWING ALL ALTERNATIVES ----------------
    st.subheader("🗺🥭 All Alternatives Map (Top 10 Highlighted)")
    ...
    for _,row in df_all.iterrows():
        ...
        if row["Name"] in df_top10["Name"].values:
            color = "gold"
            weight = 4
        else:
            color = "blue"
